...
@Singleton
class SessionStore @Inject constructor(database: LolAppDb, private val dispatchers: AppDispatchers) :
  SqldelightStore<SessionQueries>(database.sessionQueries) {

  suspend fun insert(value: SessionTable) =
    withContext(dispatchers.io) { queries.insert(value.copy(id = SessionId)).await() }

  fun observe(): Flow<SessionTable?> = queries.get().asFlow().mapToOneOrNull(dispatchers.io)

  fun observeEmailAddress(): Flow<String?> =
    queries.emailAddress().asFlow().mapToOneOrNull(dispatchers.io)
  ...
  suspend fun delete() = withContext(dispatchers.io) { queries.delete().await() }
}
