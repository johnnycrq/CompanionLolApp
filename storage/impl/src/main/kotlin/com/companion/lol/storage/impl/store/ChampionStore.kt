...
@Singleton
class ChampionStore @Inject constructor(database: LolAppDb, private val context: DatabaseContext) :
  ...
  fun observeAllWithFavorites(): Flow<List<ChampionWithFavoritesView>> =
    queries.findAll().asFlow().mapToList(context)

  fun observeWithFavoritesById(championId: ChampionId): Flow<ChampionWithFavoritesView> =
    queries.findById(championId).asFlow().mapToOne(context)

  suspend fun findKeyNameById(championId: ChampionId): String =
    withContext(context) { queries.findKeyNameById(championId).executeAsOne() }
}
